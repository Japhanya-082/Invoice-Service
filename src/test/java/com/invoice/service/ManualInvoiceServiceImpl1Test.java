...
class ManualInvoiceServiceImpl1Test {
    ...
    void saveInvoice_create_success() {
        ...
        when(invoiceRepository.save(any(ManualInvoice.class)))
                .thenAnswer(inv -> {
    ...
    void deleteInvoice_notFound_throws() {
        ...
        RuntimeException ex = assertThrows(RuntimeException.class,
                () -> service.deleteInvoice(999L, ADMIN_ID));
        assertTrue(ex.getMessage().contains("not found") || ex.getMessage().contains("unauthorized"),
    ...
    void resolveSortField_unknown_returnsInvoiceDate() {
        ...
        // We verify no exception is thrown and repository is called with a valid Pageable.
        ...
        org.springframework.data.domain.Page<ManualInvoice> emptyPage =
                new org.springframework.data.domain.PageImpl<>(new ArrayList<>());
        when(invoiceRepository.findByAdminIdAndVendorTypeIgnoreCase(
                eq(ADMIN_ID), anyString(), any())).thenReturn(emptyPage);

        org.springframework.data.domain.Page<ManualInvoice> result =
                service.getInvoicesByAdminAndVendorTypestatusinvoicestatus(dto);

        assertNotNull(result);
        // Verify the repository was called — confirming sort field resolved without error
