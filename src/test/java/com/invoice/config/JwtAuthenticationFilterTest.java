...
/**
 * Unit tests for {@link TenantFilter} — the JWT-parsing Servlet filter used
 * in this service.
 *
 * <p>Note: In this microservice, JWT authentication/authorisation is handled
 * by the API-gateway / login service.  The TenantFilter only extracts the
...
class JwtAuthenticationFilterTest {
    ...
    private static final String TEST_SECRET =
            "8f2c9a6d1e4b7c3f5a0d9e2b6c8f1a4e7d0c3b5a9f2e6d1c4b7a8e0f3d5c2b1";
    ...
    private String buildExpiredToken() {
        return Jwts.builder()
                .setSubject("testuser")
                .claim("companyDomain", "narvee.com")
    ...
    void filter_noAuthHeader_passesThrough() throws Exception {
        ...
        assertNull(TenantContext.getCurrentTenant(),
                "TenantContext must be cleared after filter execution");
    ...
    void filter_expiredJwt_passesThrough() throws Exception {
        ...
        assertNull(TenantContext.getCurrentTenant(),
                "TenantContext must remain empty after an expired-token failure");
        // HTTP response is not modified to 401 by this filter
        assertEquals(200, response.getStatus(),
                "TenantFilter should not change the HTTP status for expired tokens");
    }
    ...
    void filter_malformedJwt_passesThrough() throws Exception {
        ...
        assertNotNull(chain.getRequest(), "Filter chain should have been called");
        assertNull(TenantContext.getCurrentTenant(),
                "TenantContext must remain empty after a malformed-token failure");
        assertEquals(200, response.getStatus(),
                "TenantFilter should not change the HTTP status for malformed tokens");
    }
}
