...
public class SchemaProvisioningController {
	 ...
	 * This picks up any new entity tables Hibernate just created via
	 * ddl-auto=update.
	 */
	...
	/**
	 * Provision (or re-provision) a single tenant schema. Idempotent — safe to call
	 * multiple times.
	 */
	...
	public ResponseEntity<String> provisionSchema(@PathVariable("schemaName") String schemaName) {
		...
		try (Connection conn = rawDataSource.getConnection()) {
			...
			return ResponseEntity
					.ok("Provisioned " + cloned.size() + " tables into '" + schemaName + "' for Invoice-Service");
		} catch (Exception e) {
