...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
	...
	@Query("SELECT COALESCE(SUM(p.amount),0) FROM Payment p "
			...
			+ "AND p.status = com.invoice.entity.Payment.Status.POSTED " + "AND p.deletedAt IS NULL")
	...
	Page<Payment> findByAdminIdAndDateRange(@Param("adminId") Long adminId, @Param("from") LocalDate from,
	...
	@Query("SELECT SUM(p.amount) FROM Payment p " + "WHERE p.adminId = :adminId "
			+ "AND p.status = com.invoice.entity.Payment.Status.POSTED " + "AND p.deletedAt IS NULL "
			+ "AND p.invoiceId IN (" + "  SELECT i.id FROM ManualInvoice i "
	...
	List<Payment> findRecentPayments(@Param("adminId") Long adminId, org.springframework.data.domain.Pageable pageable);
}
