...
public class PaymentServiceImpl implements PaymentService {
    ...
    public Payment recordPayment(PaymentRequest request) {
        ...
        ManualInvoice invoice = invoiceRepository.findByIdAndAdminId(request.getInvoiceId(), adminId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Invoice " + request.getInvoiceId() + " not found in this tenant"));

        if (request.getPaymentReference() != null && !request.getPaymentReference().isBlank()
                && paymentRepository.existsByAdminIdAndPaymentReferenceIgnoreCase(adminId, request.getPaymentReference())) {
        ...
        log.info("Payment recorded: id={} invoice={} amount={} adminId={}",
                saved.getPaymentId(), invoice.getId(), saved.getAmount(), adminId);
        return saved;
    }
    ...
    public Payment voidPayment(Long paymentId, String reason) {
        ...
        Payment payment = paymentRepository.findByPaymentIdAndAdminId(paymentId, adminId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Payment " + paymentId + " not found in this tenant"));

        if (payment.getStatus() == Payment.Status.VOIDED) {
            return payment;
        }
        payment.setStatus(Payment.Status.VOIDED);
        payment.setRemarks((payment.getRemarks() == null ? "" : payment.getRemarks() + " | ")
                + "Voided: " + (reason == null ? "(no reason)" : reason));
        Payment saved = paymentRepository.save(payment);
        log.info("Payment voided: id={} invoice={} adminId={}",
                saved.getPaymentId(), saved.getInvoiceId(), adminId);
        return saved;
    }
    ...
    public List<Payment> getPaymentsForInvoice(Long invoiceId) {
        ...
        return paymentRepository
                .findByInvoiceIdAndAdminIdAndDeletedAtIsNullOrderByPaymentDateAsc(invoiceId, adminId);
    }
    ...
    public BigDecimal getInvoiceBalance(Long invoiceId) {
        ...
        ManualInvoice invoice = invoiceRepository.findByIdAndAdminId(invoiceId, adminId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Invoice " + invoiceId + " not found in this tenant"));
